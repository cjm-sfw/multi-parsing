...
class Resize(object):
    ...
    def _resize_parsings(self, results):
        for key in results.get('parsing_fields', []):
            if results[key] is None:
                continue
            if self.keep_ratio:
                parsings = [
                    mmcv.imrescale(
                        parsing, results['scale_factor'], interpolation='nearest')
                    for parsing in results[key]
                ]
            else:
                parsing_size = (results['img_shape'][1], results['img_shape'][0])
                parsings = [
                    mmcv.imresize(parsing, parsing_size, interpolation='nearest')
                    for parsing in results[key]
                ]
            results[key] = np.stack(parsings)
...
class RandomFlip(object):
    ...
    def __call__(self, results):
        ...
        if results['flip']:
            ...
            for key in results.get('parsing_fields', []):
                results[key] = np.stack([
                    mmcv.imflip(parsing, direction=results['flip_direction'])
                    for parsing in results[key]
                ])
...
class Pad(object):
    ...
    def _pad_parsings(self, results):
        ...
        for key in results.get('parsing_fields', []):
            padded_parsings = [
                mmcv.impad(parsing, pad_shape, pad_val=self.pad_val)
                for parsing in results[key]
            ]
            if padded_parsings:
                results[key] = np.stack(padded_parsings, axis=0)
            else:
                results[key] = np.empty((0, ) + pad_shape, dtype=np.uint8)
